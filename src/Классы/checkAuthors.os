...
Процедура ПриРегистрацииКомандыПриложения(ИмяКоманды, КлассРеализации) Экспорт
	...
	КлассРеализации.Опция("C error-authors", Ложь, "[*check-authors] флаг вызова ошибки при отсутствии автора")
					.Флаговый();
...
Процедура ПриПолученииПараметров(ПараметрыКоманды) Экспорт
	...
	ВызватьОшибку = ПараметрыКоманды.Параметр("error-authors", Ложь);
	
	ВызватьОшибку = Булево(ВызватьОшибку);

	Лог.Отладка("Получаю параметр <error-authors> значение <%1>", ВызватьОшибку);
...
Процедура ПередНачаломЦиклаОбработкиВерсий(ТаблицаИсторииХранилища, ТекущаяВерсия, СледующаяВерсия, МаксимальнаяВерсияДляРазбора) Экспорт
	...
	КоличествоВерсий = 0;
	МинимальнаяВерсияСОшибкой = МаксимальнаяВерсияДляРазбора + 1;

	Для Каждого СтрокаВерсии Из ТаблицаИсторииХранилища Цикл

		Если СтрокаВерсии.НомерВерсии < СледующаяВерсия Тогда
			Продолжить;
		ИначеЕсли СтрокаВерсии.НомерВерсии > МаксимальнаяВерсияДляРазбора Тогда
			Прервать;
		КонецЕсли;
		...
		Если СтрокаПользователя = Неопределено Тогда
			...
			Если МинимальнаяВерсияСОшибкой > СтрокаВерсии.НомерВерсии Тогда
				МинимальнаяВерсияСОшибкой = СтрокаВерсии.НомерВерсии;
			КонецЕсли;
	...
	Если КоличествоВерсий > 0  Тогда
		...
		Если НЕ ВызватьОшибку И МинимальнаяВерсияСОшибкой > СледующаяВерсия Тогда
			МаксимальнаяВерсияДляРазбора = МинимальнаяВерсияСОшибкой - 1;
			СтрокаОшибки = СтрокаОшибки + ". " + СтрШаблон("Будут обработаны версии до %1", МинимальнаяВерсияСОшибкой);
		КонецЕсли;
		Лог.КритичнаяОшибка(СтрокаОшибки);
